...
class SpeechBubble:
    def __init__(self, character, text, font, padding=10, max_width=300, bg_color=(255, 255, 255), text_color=(0, 0, 0), outline_color=(0, 0, 0)):
        self.character = character  # The Character instance
        ...
        # Use the character's name for the speaker
        self.speaker_name = character.name if character and character.name else "Unknown"
    ...
    def _prepare_bubble(self):
        ...
        # Add the speaker's name as a prefix
        speaker_prefix = f"{self.speaker_name}: "
    ...
    def _calculate_position(self):
        ...
        x = self.character.x + (self.character.width / 2) - (self.bubble_width / 2)
        y = self.character.y - self.bubble_height - 10
        ...
        if y < 10:
            y = self.character.y + self.character.height + 10
    ...
    def _draw_tail(self, screen, bubble_rect):
        ...
        char_center_x = self.character.x + (self.character.width / 2)

        if bubble_rect.bottom < self.character.y:
            # Bubble is above character
            points = [
                (char_center_x, self.character.y),  # Tip of the tail
                (bubble_rect.centerx - 10, bubble_rect.bottom),  # Left corner of the tail
                (bubble_rect.centerx + 10, bubble_rect.bottom),  # Right corner of the tail
            ]
        else:
            # Bubble is below character
            points = [
                (char_center_x, self.character.y + self.character.height),  # Tip of the tail
                (bubble_rect.centerx - 10, bubble_rect.top),  # Left corner of the tail
                (bubble_rect.centerx + 10, bubble_rect.top),  # Right corner of the tail
            ]
