...
# Game instructions
instructions = [
    {"text": "Oh no! Her walls are up!"},
    {"text": "Use the hammer to break them down!"},
]
...
def handle_dialogue_state(dt, game_vars): # Handle the dialogue state logic
    ...
    # Draw instructions
    for i, instr in enumerate(instructions): # Loop through instructions
        draw_text(instr["text"], font_small, WHITE, 10, 10 + i * 30) # Draw each instruction with a vertical offset
    
    # Draw flash
    if game_vars["flash_active"]: # If flash effect is active
        alpha = max(0, min(255, 255 - (game_vars["flash_timer"] / FLASH_DURATION) * 255)) # Calculate alpha value for flash effect
...
def handle_wall_game_state(dt, game_vars):
    """Handle the wall game state logic"""
    # Update player
    tim.move(dt)
    tim.update(dt)
    wall.update(dt)
    ...
    # Draw helper texts
    draw_text("Her walls are up! You must break through!", font_small, WHITE, 10, 10)
    draw_text("Her spirit cries for true connection!", font_small, WHITE, 10, 40)
    draw_text("This is your destiny!", font_small, WHITE, 10, 40)

def handle_ending_state(dt, game_vars):
    """Handle the ending state logic"""
    # Update papers
    for paper in game_vars["papers"][:]: # Loop through a copy of the papers list
        if paper.update():
            game_vars["papers"].remove(paper)
    
    # Draw papers
    for paper in game_vars["papers"]:
        paper.draw()
    ...
    # Draw ending text
    center_x = WIDTH // 2
    draw_text("Congratulations!", font_large, GOLD, center_x, 100, True)
    draw_text("You've earned....her love", font_medium, WHITE, center_x, 150, True)
    draw_text("and now you're stuck with her!", font_medium, WHITE, center_x, 190, True)
    draw_text("Hope you like CVS runs at midnight!", font_medium, WHITE, center_x, 230, True)
    draw_text("Press ESC to quit", font_small, WHITE, center_x, HEIGHT - 50, True)
